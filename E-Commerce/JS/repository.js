...
function atualizarPaginaProdutos() {
    ...
    sectionProdutos.innerHTML = '';  // Limpa os produtos antes de atualizar

    let contagemProdutos = 0;
    let linhaProdutos = null;

    produtos.forEach(produto => {
        // Se chegarmos a 4 produtos, cria uma nova linha
        if (contagemProdutos % 4 === 0) {
            linhaProdutos = document.createElement('div');
            linhaProdutos.classList.add('linha_produtos');  // Classe para a linha de produtos
            sectionProdutos.appendChild(linhaProdutos);
        }
...
<div class="apenasPozi">
    <span class="nomeproduto">${produto.nome}</span>
    <img src="edit_1159633.png "class="editar" onclick="editarProduto('${produto.id}')">
    <img src="trash_757306.png" class="excluir" onclick="excluirProduto('${produto.id}')">  
</div>
...
<div class="acoes_produto">
        ...
        // Adiciona o produto à linha atual
        linhaProdutos.appendChild(produtoDiv);

        contagemProdutos++;  // Aumenta o contador de produtos
    });
}
